...
const function getDominantColor(url: string){
    ...
    return (
        json.dominant_color.vibrant?.hex || json.dominant_colors.vibrant_light?.hex
    )
}
...
const VERTICAL_LINE_CLASSES = 'absolute top-0 h-full stroke-2 text-stone-300 transition-colors group-hover:text-strong-400'
...
export async function SkateboardProduct({id}: Props) {
    ...
    <div className='group relative mx-auto w-full max-w-72 px-8 pt-4'>
        ...
        <div className="flex items-center justicy-between ~text-sm/2xl">
            <span>{price}</span>
